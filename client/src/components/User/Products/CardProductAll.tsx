...
const CardProductAll = ({
  ...
  return (
   <div className="group w-full max-w-[220px] sm:max-w-[180px] md:max-w-[200px] lg:max-w-[220px] min-h-[355px] bg-util rounded-lg overflow-hidden shadow-[0px_1px_4px_0px_rgba(255,_138,_0,_0.25)] cursor-pointer">
  <div className="relative z-40 overflow-hidden">
    <Link to={`/${category}/${slugify(nameProduct)}`}>
      <img
        src={imageProduct}
        alt="product-image"
        className="w-full max-h-[260px] md:max-h-[220px] sm:max-h-[180px] object-cover group-hover:scale-110 group-hover:shadow-lg transition-transform duration-500 ease-in-out"
      />
    </Link>
    {/* Wishlist */}
    <button
  ...
  <Link to={`/${category}/${slugify(nameProduct)}`}>
    <div className="px-3.5 py-3 space-y-3 md:space-y-2 sm:py-2">
      <p className="line-clamp-1 text-sm md:text-base sm:text-xs">{nameProduct}</p>
      <div className="flex gap-2 text-sm md:text-xs sm:text-xs">
        {saleVariant ? (
          <>
