...
const ListVouchersAdmin = ({ setOpenModal, reset, setStatus, setCodeVoucher, setIdVoucher, setAddOrUpdate }: Props) => {
  ...
  const handleRemoveVoucher = async (id: number) => {
    console.log("Attempting to delete voucher with ID:", id); // Debugging line
    try {
    await instance.delete(`coupon/${id}`);
      ...
      toast.error(error.response?.data?.message || "An error occurred while deleting the voucher.");
    }
  };
  const handleUpdateStatus = async (id: number, statusVoucher: boolean) => {
    console.log('Voucher ID:', id);
    try {
      const { data } = await instance.patch(`coupon/${id}`, {
